...
``` vue
<template>
  <div class="jc-tinymce-container" :style="{ width: calcContentW }" v-loading="loading">
...
const skinName = VITE_THEME === 'dark' ? 'oxide-dark' : 'oxide'
  ...
  const initOptions = computed((): RawEditorOptions => {
    ...
    const mergeOption = readonly ? Object.assign(options, { inline: true }) : options
    ...
    return {
      ...
      content_css: [
        !isInlineMode.value
          ? `/tinymce/skins/content/${skinName === 'oxide-dark' ? 'dark' : 'default'}/content.css`
          : '',
      ...
      images_upload_handler: (info, progress) => imageHandle(info, progress, loading),
 ...
 * @returns
 */
export const imageHandle = async (info: BlobInfo, progress: ProgressFn, loading: Ref<boolean>) => {
  try {
    // 获取文件流
